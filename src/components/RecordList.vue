  ...
  created () {
    for (let list in this.lists) {
      ...
      if (typeof(this.lists[list].date) == "object") {
        let date1 = new Date(this.lists[list].date[0].replace(/\s/, 'T'))
        let date2 = this.lists[list].date[1] == "현재" ? new Date().replace(/\s/, 'T') : new Date(this.lists[list].date[1].replace(/\s/, 'T'))
